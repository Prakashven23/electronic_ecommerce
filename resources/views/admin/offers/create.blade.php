...
<form method="POST" action="{{ route('admin.offers.store') }}">
    ...
    <div class="mb-3">
        ...
        <select class="form-control @error('scope') is-invalid @enderror" id="scope" name="scope" required onchange="toggleScopeSelectors()">
            <option value="category" {{ old('scope') == 'category' ? 'selected' : '' }}>Category-wise</option>
            <option value="product" {{ old('scope') == 'product' ? 'selected' : '' }}>Product-wise</option>
        </select>
        @error('scope')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
    <div class="mb-3" id="category_selector" style="display: {{ old('scope', 'category') == 'category' ? 'block' : 'none' }};">
    ...
    <div class="mb-3" id="product_selector" style="display: {{ old('scope') == 'product' ? 'block' : 'none' }};">
    ...
    <div class="mb-3">
        ...
        <input type="number" step="0.01" class="form-control @error('minimum_order_value') is-invalid @enderror" id="minimum_order_value" name="minimum_order_value" value="{{ old('minimum_order_value') }}" required>
